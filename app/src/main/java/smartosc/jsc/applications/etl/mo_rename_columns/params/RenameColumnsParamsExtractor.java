...
public class RenameColumnsParamsExtractor implements NodeParamsExtractor<List<ColumnModel>> {
    ...
    public List<ColumnModel> extractParams(String params) throws Exception {
        ...
        for (ColumnModel column : convertedParams) {
            if (column.getOldColumn().isEmpty()) {
                throw new RuntimeException("old column is null");
            }

            if (column.getNewColumn().isEmpty()) {
                throw new RuntimeException("New column is null");
            }
        }
